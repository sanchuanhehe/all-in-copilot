...
`);
	...
	private async collectConfig(projectName?: string): Promise<ProjectConfig> {
		...
		const name =
			projectName ||
			(await this.ask("📛 Project name (e.g., glm-copilot): ", "my-copilot", (v) =>
				/^[a-z0-9-]+$/.test(v) ? null : "Use lowercase letters, numbers, and hyphens only"
			));

		// Display name
		const displayName = await this.ask("📝 Display name (shown in VS Code): ", this.toDisplayName(name));

		// Description
		const description = await this.ask("📄 Description: ", `${displayName} - Custom LLM provider for VS Code Chat`);

		// Publisher
		const publisher = await this.ask("👤 Publisher name: ", "my-publisher");
		...
		const presetIndex = await this.ask(`\nEnter number (1-${presetKeys.length}): `, "1", (v) => {
			const n = parseInt(v);
...
`;
		...
		content = content.replace(/> .+/m, `> ${config.description}`);
